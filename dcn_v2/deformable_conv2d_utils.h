...
Status CheckFormatConstraintsOnShape(const TensorFormat tensor_format, const ShapeHandle shape_handle,
                                     const string &tensor_name, InferenceContext *ctx) {
    if (tensor_format == FORMAT_NCHW_VECT_C) {
        const int num_dims = ctx->Rank(shape_handle);
        DimensionHandle vect_dim = ctx->Dim(shape_handle, GetTensorInnerFeatureDimIndex(num_dims, tensor_format));
        DimensionHandle unused_vect_dim;
        TF_RETURN_IF_ERROR(ctx->WithValue(vect_dim, 4, &unused_vect_dim));
    }
    return Status::OK();
}
Status DimensionsFromShape(ShapeHandle shape, TensorFormat format, DimensionHandle *batch_dim,
                           gtl::MutableArraySlice<DimensionHandle> spatial_dims, DimensionHandle *filter_dim,
    ...
    *batch_dim = ctx->Dim(shape, GetTensorBatchDimIndex(rank, format));
    for (int spatial_dim_index = 0; spatial_dim_index < spatial_dims.size(); ++spatial_dim_index) {
        spatial_dims[spatial_dim_index] = ctx->Dim(shape, GetTensorSpatialDimIndex(rank, format, spatial_dim_index));
    }
    *filter_dim = ctx->Dim(shape, GetTensorFeatureDimIndex(rank, format));
    if (format == FORMAT_NCHW_VECT_C) {
        TF_RETURN_IF_ERROR(ctx->Multiply(*filter_dim, ctx->Dim(shape, GetTensorInnerFeatureDimIndex(rank, format)),
                                         filter_dim));
    }
    return Status::OK();
}
Status ShapeFromDimensions(DimensionHandle batch_dim, gtl::ArraySlice<DimensionHandle> spatial_dims,
                           DimensionHandle filter_dim, TensorFormat format, InferenceContext *ctx, ShapeHandle *shape) {
    const int32 rank = GetTensorDimsFromSpatialDims(spatial_dims.size(), format);
    std::vector<DimensionHandle> out_dims(rank);
    out_dims[GetTensorBatchDimIndex(rank, format)] = batch_dim;
    for (int spatial_dim_index = 0; spatial_dim_index < spatial_dims.size(); ++spatial_dim_index) {
        out_dims[GetTensorSpatialDimIndex(rank, format, spatial_dim_index)] = spatial_dims[spatial_dim_index];
    }
    if (format == FORMAT_NCHW_VECT_C) {
        TF_RETURN_IF_ERROR(ctx->Divide(filter_dim, 4, true, &out_dims[GetTensorFeatureDimIndex(rank, format)]));
...
Status InitDeformableConv2DParameters(const OpKernelConstruction *ctx, DeformableConv2DParameters *params) {
    ...
    TF_RETURN_IF_ERROR(ctx->GetAttr("deformable_groups", &params->deformable_groups));
    ...
    TF_REQUIRES(dilation_h > 0 && dilation_w > 0,
...
Status ComputeDeformableConv2DDimension(const DeformableConv2DParameters &params,
                                        ...
                                        int flag) {
    TF_REQUIRES(input.dims() == 4, errors::InvalidArgument("input must be 4-dimensional",
                                                           input.shape().DebugString()));
    TF_REQUIRES(filter.dims() == 4, errors::InvalidArgument("filter must be 4-dimensional: ",
