...
def block1(x, filters, kernel_size=3, stride=1, conv_shortcut=True, 
  ...
  if not isinstance(use_dcn, bool):
    raise ValueError("use_dcn must be a boolean.")
...
def stack1(x, filters, blocks, stride1=2, use_dcn=False, skip_interval=False, name=None):
  ...
  for i,j in zip(range(2, blocks + 1), range(1,blocks)):
    ...
    if j % 3 == 0 and use_dcn and skip_interval:
      ...
      x = block1(x, filters, conv_shortcut=False, use_dcn=False, 
                 name=name + '_block' + str(i))
  return x
...
def stack2(x, filters, blocks, stride1=2, use_dcn=False, skip_interval=False, name=None):
  ...
  Returns:
      Output tensor for the stacked blocks.
  """
  x = block2(x, filters, conv_shortcut=True, use_dcn=use_dcn, 
             name=name + '_block1')
  ...
  if (j+1) % 3 == 0 and use_dcn:
    x = block2(x, filters, stride=stride1, use_dcn=True, 
               name=name + '_block' + str(blocks))
  else:
    x = block2(x, filters, stride=stride1, use_dcn=False, 
               name=name + '_block' + str(blocks))
  return x
...
def stack3(x, filters, blocks, stride1=2, groups=32, use_dcn=False, skip_interval=False, name=None):
  ...
  Returns:
    Output tensor for the stacked blocks.
  """
  x = block3(x, filters, stride=stride1, groups=groups, use_dcn=use_dcn, 
             name=name + '_block1')
...
def ResNet50(include_top=True,
  ...
  def stack_fn(x):
    ...
    x = stack1(x, 256, 6, use_dcn=dcn_layers[2], name='conv4')
